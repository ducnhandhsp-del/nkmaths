...
function getData() {
  try {
    ...
    var studentsRaw = getRows(SHEETS.HOCSINH);
    var teachersRaw = getRows(SHEETS.GIAOVIEN);
    var classesRaw = getRows(SHEETS.LOPHOC);
    var regsRaw = getRows(SHEETS.DANGKYLOP);
    var lessonsRaw = getRows(SHEETS.BUOIHOC);
    var attRaw = getRows(SHEETS.DIEMDANH);
    var paymentsRaw = getRows(SHEETS.HOCPHI);
    var expensesRaw = getRows(SHEETS.CHIPHI);
    ...
    return {
      ...
      tv: tv,
      hl: [],
      leaveRequests: readLeaveRequestsAdapter(),
      summary: {
...
function readLeaveRequestsAdapter() {
  var rows = getRows(SHEETS.NGHIHOC);
  var studentMap = buildMap(getRows(SHEETS.HOCSINH), 'MaHS');
  var lessonMap = buildMap(getRows(SHEETS.BUOIHOC), 'MaBuoi');
...
function setupSheetsIfMissingOnly() {
  ...
  names.forEach(function(name) {
    if (!sh(name)) ensureSheet(name, HEADERS[name]);
  });
  applyTextFormats();
}
